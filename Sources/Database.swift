...
class DB {
	...
	func getData(from: Int = 1990, to: Int = 2020) -> String {
		var contentDict = [String: Int]()
		...
		do {
			...
			let demoStatement = "SELECT x, y FROM demo WHERE x > :1 AND x < :2 ORDER BY id ASC"

			try sqlite.forEachRow(statement: demoStatement, doBindings: {

				(statement: SQLiteStmt) -> () in

				try statement.bind(position: 1, from)
				try statement.bind(position: 2, to)

				}, handleRow: {(statement: SQLiteStmt, i:Int) -> () in
					contentDict[String(statement.columnInt(position: 0))] = statement.columnInt(position: 1)
			})
		...
		do {
			return try contentDict.jsonEncodedString()
		} catch {
			print(error)
			return "[\(error)]"
		}
